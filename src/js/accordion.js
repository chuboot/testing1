...
function accordion() {
  ...
  for (let index = 0; index < accordionContainer.length; index++) {
    ...
    addClass(
      button,
      "absolute blick md:hidden right-0 transform -translate-y-1/2 focus:outline-none transition duration-200 rotate-0"
    );
    button.style.top = "50%";
...
`;
    ...
    el.getElementsByTagName("h2")[0].append(button);
  }
}
